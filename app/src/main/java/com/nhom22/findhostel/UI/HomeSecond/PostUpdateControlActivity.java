...
public class PostUpdateControlActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (dataBundle != null) {
            idCurrentPost = dataBundle.getInt("id");
        ...
        houseNumberEditText.setText(p.getAddress().getHouseNumber());
        autoCitiesField.setText(p.getAddress().getCities().getName());
        autoDistrictField.setText(p.getAddress().getDistricts().getName());
        autoSubDistrictField.setText(p.getAddress().getSubDistrics().getName());
        autoStreetField.setText(p.getAddress().getStreets().getName());


        namePostEditText.setText(p.getPostName());
        pricePost.setText(String.valueOf(p.getPrice()));
        desPost.setText(p.getDescription());
        ...
        btnAddPost.setOnClickListener(v -> {
            ...
            // Set the address
            Address address = postsService.getAddressByPostId(idCurrentPost);
            ...
            runOnUiThread(() -> {
                ...
                List<Address> lstAddress = addressService.getAllAddress();
                ...
                if (!addressExists) {
                    long addressId = addressService.updateAddress(address);
                    if (addressId > 0) {
                        Toast.makeText(this, "Sửa địa chỉ thành công", Toast.LENGTH_SHORT).show();
                        p.setAddress(address);
                    }
                }
                // Set the post name
                p.setPostName(namePost);

                // Set the price
                p.setPrice(Float.parseFloat(priceText));

                // Set the description
                p.setDescription(desPostA);
                ...
                p.setTimeFrom(currentDate);
                p.setTimeTo(futureDate);
                ...
                // Update the post
                boolean postId = postsService.updatePost(p);
                if (postId == true) {
                    Toast.makeText(this, "Sửa bài viết thành công", Toast.LENGTH_SHORT).show();
                    ...
                    for (Images images : imagesList) {
                        try {
                            detail_imageService.addADetailImage(images.getId(), idCurrentPost);
                        } catch (ParseException e) {
                            throw new RuntimeException(e);
                        }
                    }
                    ...
                    for (Furniture furniture : furnitureListChoose) {
                        try {
                            detailFurniture.setPosts(postsService.getPostById(idCurrentPost));
                        } catch (ParseException e) {
                            throw new RuntimeException(e);
                        }
                    ...
                    for (Detail_Utilities detailUtilities : detail_utilitiesList) {
                        if (detailUtilities.getPrice() != 0.0) {
                            try {
                                detailUtilities.setPosts(postsService.getPostById(idCurrentPost));
                            } catch (ParseException e) {
                                throw new RuntimeException(e);
                            }
    ...
    private Uri getUriFromByte(byte[] byteArray, String fileName) throws IOException {
